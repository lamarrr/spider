...
def get_products(proxies=None):
    ...
    try:
        ...
        if config.use_proxies:
            print("[JDSports] Fetching Products...")
            jdsports_response = requests.get("https://www.jdsports.co.uk/campaign/new+in/?facet-new=latest&sort=latest",headers=headers,proxies=proxies)
            print("[JDSports] Finished Fetching Products")
        else:
            print("[JDSports] Fetching Products...")
            jdsports_response = requests.get("https://www.jdsports.co.uk/campaign/new+in/?facet-new=latest&sort=latest",headers=headers,proxies=config.test_proxy)
            print("[JDSports] Finished Fetching Products")
        jdsports_document=jdsports_response.text
        return jdsports_document
    except requests.ConnectionError:
        print("\n[JDSports] ----------Connection Error\n")
        exit()
