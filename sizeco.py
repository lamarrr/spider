...
def get_products(proxies=None):
    ...
    try:
        ...
        if config.use_proxies:
            print("[Size] Fetching Products...")
            sizeco_response = requests.get("https://www.size.co.uk/campaign/New+In/latest/?facet-categories=size&st_sort_order=latest",headers=headers,proxies=proxies)
            print("[Size] Finished Fetching Products")
        else:
            print("[Size] Fetching Products...")
            sizeco_response = requests.get("https://www.size.co.uk/campaign/New+In/latest/?facet-categories=size&st_sort_order=latest",headers=headers,proxies=config.test_proxy)
            print("[Size] Finished Fetching Products")
